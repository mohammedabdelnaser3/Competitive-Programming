...
int mode_inv(int b, int m){
    return Power(b, mod - 2);
}
void add_front(int &hash, char c){
    hash = (hash * base + c) % mod;
}

void add_back(int &hash, char c, int size){
    hash += (c * power[size]) % mod;
    hash %=mod;
}

void remove_front(int &hash, char c){
    hash = ((hash - c + mod) % mod) * mode_inv(base, mod) % mod;
}

void remove_back(int &hash, char c, int size){
    hash = (hash - (c * power[size] % mod) + mod) % mod;
}

void blow_it()  {
    int n, k = rng(1e10, 1e12), h = rng(70, 140), tHash = 0, sHash = 0, m;
    string t, s;
    cin >> s >> t;
    n = s.size();
    m = t.size();

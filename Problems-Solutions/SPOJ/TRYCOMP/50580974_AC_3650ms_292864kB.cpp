...
struct Trie{
    ...
    pair<int, string> insert(const string& s, int pos){
        if(pos != s.size()) {
            auto p = children[char(s[pos])].insert(s, pos + 1);
            if(p.first > mx.first)
                mx = p;
            else if(p.first == mx.first)
                mx = (p.second < mx.second ? p : mx);
            return mx;
        }
        else{
            ++isWord;
    ...
    pair<int, string> search(const string &s, int pos) {
        if (pos == s.size())
            return mx;

        if (children.find(s[pos]) != children.end())
            return children[s[pos]].search(s, pos + 1);

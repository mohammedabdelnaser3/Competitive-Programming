...
pii query(int s, int e, int node, int l, int r){
    ...
    if(s >= l && e <= r)
        return tree[node];
    int md = (s + e) >> 1;
    return max(query(s, md, node << 1, l, r), query(md + 1, e,( node << 1)| 1, l, r));
}
...
void blow_it() {
    ...
    while(m--){
        ...
        if(op == 'U')
            ...
            dbg(l, r);
            pii ans = query(0, n - 1, 1, l - 1, r - 1);
            cout << ans.first + max(
                query(0 , n - 1, 1, l - 1, ans.second - 1).first,
                 query(0, n - 1, 1, ans.second + 1, r - 1).first) << '\n';
        }
    }

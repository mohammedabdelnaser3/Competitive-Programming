...
void blow_it()  {
    string s, h;
    precomputePowers(1e6);
    while(cin >> s){
        int n = s.size();

        vector<int>pre(n + 1), suffix(n + 1);
        for (int i = 0; i < n; ++i)
            pre[i + 1] = (pre[i] + (s[i] * power[i] % mod)) % mod;

        for (int i = n - 1; i >= 0; --i)
            suffix[i] = (suffix[i + 1] + s[i] * power[n - i - 1]) % mod;
        dbg(pre);
        dbg(suffix);
        int idx = n;
        for (int i = n - 1; i >= 0 ; --i){
            if((pre[n] - pre[i] + mod) % mod == suffix[i] * power[i] % mod)
                idx = i;
        }

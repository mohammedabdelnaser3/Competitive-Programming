...
void blow_it() {
    int h, w, n;
    cin >> h >> w >> n;
    set<int>s1 = {0, h}, s2 = {0, w};
    set<pair<int,pii>>st1 = {{h, {0, h}}}, st2 = {{w, {0, w}}};
    for (int i = 0; i < n; ++i) {
        char c;
        int x;
        cin >> c >> x;
        if(c == 'H'){
            ...
            if(it != s2.end()){
                st2.erase({*it - *prev(it),{*prev(it), *it}});
                st2.insert({x - *prev(it),{ *prev(it), x}});
                st2.insert({*it - x, {x, *it}});
                s2.insert(x);
            }
        }else{
            auto it = s1.upper_bound(x);
            if (it != s1.end()) {
                st1.erase({*it - *prev(it), {*prev(it), *it}});
                st1.insert({x - *prev(it), {*prev(it), x}});
                st1.insert({*it - x, {x, *it}});
                s1.insert(x);
            }
        }

        cout << st1.rbegin()->first * st2.rbegin()->first << '\n';
    }
}

...
bitset<30> build1(int l = 0, int r = n - 1, int node = 1) {
    if (l == r) {
        tree1[node] = 1 << (v[l] - 'a');
        return tree1[node];
    }
...
void update(int l,int r, char v, int node = 1, int s = 0, int e = n - 1){
    // prop(node, s, e);
    if (s > r || e < l) return;
    if(s >= l && e <= r){
        tree1[node] = 1 << (v - 'a');
...
int query(int l, int r, int node = 1, int s = 0, int e = n - 1){
    ...
    return gcd(query(l, r, node << 1, s, (s + e) / 2) , query(l, r, (node << 1) | 1, (s + e) / 2 + 1, e));
}
bitset<30> query1(int l, int r, int node = 1, int s = 0, int e = n - 1) {
    if (s > r || e < l) return {};
...
void blow_it()  {
   ...
   /* for (int i = 0; i < 4 * n; ++i){
    ...
    vi idx;
    for (int i = 0; i < m; ++i)
        cin >> v[i], idx.emplace_back(v[i]);

    vector<pair<char,vi>> querys;
    while(q--){
        int a, b;
        char c;
        cin >> c >> a >> b;
        querys.emplace_back(c, vi{a, b}); 
        if(c == '?')
            idx.emplace_back(a), idx.emplace_back(b);
        else
            idx.emplace_back(b);
    }*/
...
/*    for (int i = 0; i < m; ++i){
        ...
        update(get(v[i]), get(v[i]), 1);
    }
    
    for (int i = 0; i < querys.size(); ++i){
        if(querys[i].first == '?'){
            dbg(querys[i]);
            cout << query(get(querys[i].second[0]), get(querys[i].second[1])) << '\n';
        }
        else{
            int k =  querys[i].second[0] - 1;
            update(get(v[k]), get(v[k]), -1);
            v[k] = querys[i].second[1];
            update(get(v[k]), get(v[k]), 1);
